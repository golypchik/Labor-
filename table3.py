...
def create_dynamic_tables3(doc, selected_periods, periods_db_path, logger_stats_db_path, 
    ...
    for period in selected_periods:
        ...
        logger_stats_cursor.execute("""
            SELECT MIN(min_value), MAX(max_value), 
                (SELECT logger_number FROM logger_stats WHERE period_id = ? AND data_type = 'temperature' AND logger_type != 'external' ORDER BY min_value ASC LIMIT 1),
                (SELECT logger_number FROM logger_stats WHERE period_id = ? AND data_type = 'temperature' AND logger_type != 'external' ORDER BY max_value DESC LIMIT 1)
            FROM logger_stats 
            WHERE period_id = ? AND data_type = 'temperature' AND logger_type != 'external'
        ...
        logger_stats_cursor.execute("""
            SELECT MIN(min_value), MAX(max_value),
                (SELECT logger_number FROM logger_stats WHERE period_id = ? AND data_type = 'humidity' AND logger_type != 'external' ORDER BY min_value ASC LIMIT 1),
                (SELECT logger_number FROM logger_stats WHERE period_id = ? AND data_type = 'humidity' AND logger_type != 'external' ORDER BY max_value DESC LIMIT 1)
            FROM logger_stats 
            WHERE period_id = ? AND data_type = 'humidity' AND logger_type != 'external'
        """, (period_id, period_id, period_id))
    ...
    for period in selected_periods:
        ...
        # Создание таблиц для температуры и влажности
        for data_type in ['temperature', 'humidity']:
            ...
            logger_stats_cursor.execute("""
                ...
                ORDER BY 
                    CASE 
                        ...
                        WHEN logger_type = 'external' THEN 2
                        ELSE 3
                    END,
                    logger_number
            """, (period_id, data_type))
            ...
            headers = [
                ...
                f"Средняя {data_type1}, {unit}", "Соответствие критерию", None, "Выполнил", "Дата"
            ]
            ...
            # Объединяем ячейки "Соответствие критерию"
            table.cell(0, 4).merge(table.cell(0, 5))

            # Заполняем подзаголовки "да" и "нет"
            subheaders = ["да", "нет"]
            ...
            # Находим минимальное и максимальное значения только среди внутренних логгеров
            internal_values = [data for data in logger_data if data[4] == 'internal']
            if internal_values:
                min_value = min(data[1] for data in internal_values)
                max_value = max(data[2] for data in internal_values)
            else:
                min_value = None
                max_value = None
            ...
            for row, (logger_number, min_val, max_val, avg_value, logger_type) in enumerate(logger_data, start=2):
                ...
                # Запоминаем логгеры с выходом температуры за пределы
                if (
                ...
                for col in range(8):
                    ...
                    if col == 1:
                        ...
                        # Выделяем цветом только для внутренних логгеров
                        if logger_type == 'internal' and min_value is not None and min_val == min_value:
                            set_cell_background(cell, "87CEFA")

                    elif col == 2:
                        cell.text = f"{max_val:.2f}"
                        # Выделяем цветом только для внутренних логгеров
                        if logger_type == 'internal' and max_value is not None and max_val == max_value:
                        ...
                        # Внутренние логгеры: ставим "+" в колонке "да" или "нет"
                        if col == 4:
                            # Соответствие критерию (ДА)
                            if data_type == 'temperature' and temp_min is not None and temp_max is not None:
                    ...
                    elif logger_type == 'external' and col == 4:
                        # Внешний логгер: текст вместо плюсов, объединяем 5-й и 6-й столбцы
                        cell.text = "Около зоны хранения лекарственных средств"
                        cell.paragraphs[0].runs[0].bold = True
            ...
            table.cell(2, 6).text = "Сенчишина Т.А."
            ...
            table.cell(start_row + 2, 1).text = "Харевич А.И."
    ...
    run.font.name = 'Times New Roman'
    run.font.size = Pt(12)
    conclusion_heading.paragraph_format.space_before = Pt(6)  # 4 пт над текстом
    conclusion_heading.paragraph_format.space_after = Pt(6)   # 2 пт под текстом
