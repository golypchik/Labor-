...
def create_dynamic_tables5(doc, selected_periods, periods_db_path, logger_stats_db_path, 
                           temp_min, temp_max, 
                           mapping_results, conclusion, contract_date,
                           temp_homogeneity_text=None, selected_recommendations=None):
    ...
    min_temp = float('inf')
    max_temp = float('-inf')
    min_temp_logger = max_temp_logger = None

    # Номера логгеров с выходом температуры за пределы
    out_of_range_loggers = set()
    ...
    for period in selected_periods:
        ...
        for data_type in ['temperature']:
            ...
            # Заголовок таблицы
            unit = "°C"
            header_text = ('Таблица 3 – Результаты картирования по температуре в зоне хранения лекарственных средств (в холодильнике) при режиме терморегуляции №1' )
                        ...
                        if col == 4:
                            ...
                            if data_type == 'temperature':
                                if min_val < temp_min or max_val > temp_max:
                                    cell.text = "+"
                                    out_of_range_loggers.add(str(logger_number))

                    # Копим данные для расчёта средних по зоне и стабильности
                    if logger_type == 'internal' and data_type == 'temperature':
    ...
    run = table_heading.add_run("Таблица 4 – Результаты проведения исследований(в холодильнике)")
    run.font.name = 'Times New Roman'
    run.font.size = Pt(12)
    ...
    out_of_range_result = (
        f"№ логгера – {', '.join(sorted(out_of_range_loggers))}"
        if out_of_range_loggers
        else "Не выявлены"
    )

    # Средняя температура по всей зоне (холодильнику)
    ...
    # Определение данных для новой таблицы 4 (без полей влажности)
    data = [
    ...
    # Добавляем разрыв страницы после таблицы
    doc.add_page_break()
    out_of_range_result = f"№ логгера – {', '.join(sorted(out_of_range_loggers))}" if out_of_range_loggers else "Не выявлены"

    return paragraph_index, out_of_range_result
